# 학습 요약: Day 07 — BGR/RGB 색상 순서와 이미지 전처리 기본 흐름을 익힌다.
...
def main():
    ...
    rgb_preview_path = Path("outputs/d7_rgb_preview.png")
    restored_bgr_path = Path("outputs/d7_bgr_restored.jpg")
    ...
    print(restored_bgr.shape)
    print(restored_bgr.dtype)

    assert (restored_bgr == image).all()

    save_rgb_preview(rgb_image, rgb_preview_path)
    assert rgb_preview_path.exists()

    save_image(restored_bgr, restored_bgr_path)
    assert restored_bgr_path.exists()
