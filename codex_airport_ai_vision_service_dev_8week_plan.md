...
이 계획의 목표는 TODO를 빠르게 채우는 사람이 아니라, 요구사항에서 코드 구조를 추론하고 스스로 설명할 수 있는 개발자가 되는 것이다.
따라서 `개념을 하나씩` 진행한다는 말은 소스 코드 한 줄씩 정답을 알려 준다는 뜻이 아니라, **설계 개념 하나를 기능 단위로 직접 풀어 본다**는 뜻이다.

- 새 실습은 먼저 도메인 상황, 해결할 문제, 성공 조건과 실패 조건만 제시한다. 예: 공항 CCTV client가 filtering 조건을 보내고, API가 유효한 조건만 처리한다.
- 구현 전에 나는 아래를 짧게 직접 설계한다: HTTP method 선택 이유, request/response data flow, 입력·출력 자료형, validation 위치, endpoint와 helper의 책임, 테스트로 증명할 결과.
- 그 뒤에는 함수, endpoint, 또는 pytest 함수처럼 의미 있는 코드 블록을 15~30분 먼저 독립 구현한다. 이미 배운 문법은 한 줄 단위 빈칸 문제로 쪼개지 않는다.
- TODO는 책임과 성공 조건을 보여 줄 수는 있지만, 바로 앞에서 답한 field 이름·path·argument·expected value를 빈칸에 감춘 정답 제시 형태로 만들지 않는다.
- 도움이 필요할 때는 다음 순서로만 힌트를 준다: `추론 질문 → 관련 개념/도메인 힌트 → 함수·API 계약 힌트 → 정확한 코드`. 정확한 코드나 완성된 assertion은 내가 명시적으로 요청했을 때만 준다.
- 코드리뷰는 문법 정답 확인에 그치지 않고, 왜 이 구조인지, 다른 선택지는 왜 아닌지, validation은 어느 경계에서 일어나는지, 실제 공항·관제 도메인에서는 정책·권한을 어떻게 둘지를 함께 묻는다.
- 테스트는 behavior specification(입력, 기대 결과, 막아야 할 버그)을 먼저 받고 내가 직접 작성·실행한다. Codex는 내가 명시적으로 요청하기 전에는 테스트 assertion을 대신 쓰거나 pytest를 대신 실행하지 않는다.
- review 파일에는 이전 답안의 DTO field, helper 내부 로직, endpoint 연결을 미리 채워 넣지 않는다. 요구사항과 재사용 가능한 기존 모듈만 확인한 뒤 내가 독립 재구성한다.
